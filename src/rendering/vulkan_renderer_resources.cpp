...
void VulkanRenderer::updateInstanceBuffer(const octree::OctreePlanet::RenderData& renderData) {
    ...
    // std::cout << "updateInstanceBuffer called (call #" << ++callCount << ")" << std::endl;
    
    visibleNodeCount = static_cast<uint32_t>(renderData.visibleNodes.size());
    
    // static bool firstTime = true;
    // if (firstTime) {
    //     std::cout << "UpdateInstanceBuffer: " << visibleNodeCount << " visible nodes" << std::endl;
    //     firstTime = false;
    // }
    
    if (visibleNodeCount == 0) {
        // std::cout << "Warning: No visible nodes to render!" << std::endl;
        return;
    }
    ...
    for (uint32_t nodeIdx : renderData.visibleNodes) {
        ...
        instance.center = node.center;
        instance.halfSize = node.halfSize;
        ...
        switch (materialType) {
            ...
            case 1: // Rock
                ...
                break;
            case 2: // Water
                instance.color = glm::vec3(0.0f, 0.3f, 0.7f);
                break;
            case 3: // Magma
                instance.color = glm::vec3(1.0f, 0.3f, 0.0f);
                break;
            default:
                instance.color = glm::vec3(0.5f, 0.5f, 0.5f);
                break;
        }
        
        instance.materialType = materialType;
