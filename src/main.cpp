...
private:
    ...
    void init() {
        ...
        // Set initial camera position - back away from planet (1.5x radius instead of 3x)
        camera.setPosition(glm::vec3(0, 0, config.radius * 1.5f));
    ...
    void handleInput() {
        ...
        if (!imguiWantsMouse && input.scrollDelta.y != 0) {
            float zoomSpeed = 1.1f; // Multiplicative factor
        ...
        if (input.keys[GLFW_KEY_R] && !input.prevKeys[GLFW_KEY_R]) {
            camera.setPosition(glm::vec3(0, 0, config.radius * 3.0f));
            camera.lookAt(glm::vec3(0, 0, 0));
            if (!config.quiet) {
