...
private:
    ...
    void handleInput(float deltaTime) {
        ...
        if (input.mouseButtons[0]) { // Left mouse button
            if (camera->getMode() == core::CameraMode::Orbital) {
                // Rotate camera based on mouse movement
                float sensitivity = 0.01f;
                camera->orbit(input.mouseDelta.x * sensitivity, 
                             -input.mouseDelta.y * sensitivity);
            }
        }
        ...
        if (input.scrollDelta.y != 0) {
            if (camera->getMode() == core::CameraMode::Orbital) {
                camera->zoom(-input.scrollDelta.y * 0.1f);
            }
        }
        
        // Handle keyboard input for camera movement
        float moveSpeed = 1000000.0f * deltaTime; // 1000 km/s
        if (input.keys[87]) { // W key
            camera->moveForward(moveSpeed);
        }
        if (input.keys[83]) { // S key
            camera->moveForward(-moveSpeed);
        }
        if (input.keys[65]) { // A key
            camera->moveRight(-moveSpeed);
        }
        if (input.keys[68]) { // D key
            camera->moveRight(moveSpeed);
        }
        if (input.keys[81]) { // Q key
            camera->moveUp(-moveSpeed);
        }
        if (input.keys[69]) { // E key
            camera->moveUp(moveSpeed);
        }
