...
private:
    ...
    void testNodeInitialization() {
        ...
        int airCount = 0;
        for (int i = 0; i < 8; i++) {
            if (voxels[i].material == MaterialType::Air) {
                airCount++;
            }
            assert(voxels[i].density == 0.0f);
            assert(voxels[i].temperature == 0.0f);
        }
    ...
    void testMaterialPersistence() {
        ...
        // Create a test voxel
        Voxel testVoxel;
        testVoxel.material = MaterialType::Rock;
        testVoxel.density = 2700.0f;
        ...
        if (retrieved) {
            assert(retrieved->material == MaterialType::Rock);
            assert(retrieved->density == 2700.0f);
            ...
            for (const auto& voxel : voxels) {
                if (voxel.material == MaterialType::Rock) {
                    foundRock = true;
                    break;
                }
            }
    ...
    void testTraversalAndRetrieval() {
        ...
        if (nodesWithMaterials == 0) {
            ...
            for (int i = 0; i < std::min(5, (int)leafNodes.size()); i++) {
                ...
                std::cout << "      Materials: ";
                for (int j = 0; j < 8; j++) {
                    std::cout << static_cast<int>(voxels[j].material) << " ";
                }
                std::cout << std::endl;
            }
        } else {
